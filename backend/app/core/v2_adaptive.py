"""Removable adapter for the opt-in V2.27/V2.28 adaptive inference path.
...
from ..models import FinalTask, MeetingInput, PipelineDiagnostics, PipelineResult
...
class V2AdaptiveUnavailableError(RuntimeError):
    """Raised when the optional V2 implementation or its artifacts are absent."""
...
class V2AdaptiveCore:
    ...
    def process(self, meeting: MeetingInput, **options: Any) -> PipelineOutput:
        ...
        ``options`` is intentionally limited to runner metadata.  In
        particular, callers cannot inject an AI client or provider; the V2.27
        runner always constructs its disabled local client for text inference.
        """
